...
/*   Updated: 2021/04/08 15:35:53 by aldubar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t	push_pos(int nb, t_stack **stack)
{
	t_stack	*tmp;
	int		pos;
	int		prev_nb;
...
static void		calculate_b_op(t_stack **b, t_op *op, int nb)
{
	size_t	size;
	size_t	pos;
...
t_op			*calculate_less_op(t_stack **a, t_stack **b)
{
	...
	tmp = *a;
	calculate_b_op(b, less, tmp->nb);
	while (tmp)
	{
		calculate_a_op(a, op, pos);
		calculate_b_op(b, op, tmp->nb);
		if (op->op < less->op)
